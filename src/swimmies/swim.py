...
class SwimProtocol:
            ...
            try:
                ...
                with self.lock:
                    alive_members = [
                        ...
                        if member.is_alive and member.node_info.node_id != self.node_info.node_id
                    ]
    ...
    def _ping_member(self, member: SwimMember):
        ...
        def timeout_handler():
            ...
            with self.lock:
                if (member.node_info.node_id in self.members and 
                    self.members[member.node_info.node_id].is_alive):
    ...
    def _check_suspect_timeouts(self):
        ...
        with self.lock:
            for node_id, member in self.members.items():
                if (member.is_suspect and 
                    member.suspect_timeout and 
                    current_time > member.suspect_timeout):
    ...
    def _broadcast_leave(self):
        ...
        for member in alive_nodes:
            try:
                ...
                self.logger.error(f"Failed to send leave to {member.node_info.node_id}: {e}")
    ...
    def _send_message(self, message: SwimMessage, target_ip: str, target_port: int):
        ...
        try:
            ...
            self.logger.error(f"Failed to send message to {target_ip}:{target_port}: {e}")
